...
contract("Congo", ([deployer, seller, buyer]) => {
  ...
  describe("products", async () => {
    ...
    before(async () => {
      result = await congo.createListing(
        1,
        web3.utils.toWei("2.5", "ether"),
        "best iPhone ever.",
        "iPhone X",
        "https://www.imgur.com/a/imageid1",
        "kentkfeng@gmail.com",
        { from: seller }
      );
    ...
    it("creates products", async () => {
      ...
      assert.equal(
        event.imageLink,
        "https://www.imgur.com/a/imageid1",
        "image listing link is correct"
      );
      assert.equal(
        event.sellerContactDetails,
        "kentkfeng@gmail.com",
        "seller contact email is correct"
      );

      // FAILURE: Including empty strings for each argument.
      await await congo.createListing(
        0,
        web3.utils.toWei("2.5", "Ether"),
        "details2",
        "item2",
    ...
    it("create order", async () => {
      ...
      oldContractBalance = await web3.eth.getBalance(congo.address);
      ...
      result = await congo.createOrder(itemID, qtyBuy, notes, buyerEmail, {
        from: buyer,
        value: amountToSend
      });
      ...
      assert.equal(
        updatedListing.quantity.toNumber(),
        0,
        "quantity is deducted."
      );
      assert.equal(
        updatedListing.imageLink,
        "https://www.imgur.com/a/imageid1",
        "image listing link is correct"
      );
      assert.equal(
        updatedListing.sellerContactDetails,
        "kentkfeng@gmail.com",
        "seller contact email is correct"
      );
      assert.equal(
      ...
      assert.equal(
        order.sellerContactDetails,
        sellerEmail,
        "seller contact is correct"
      );
      assert.equal(
        order.buyerContactDetails,
        buyerEmail,
        "buyer contact is correct"
      );
      assert.equal(order.orderStatus, ordStat, "order status is correct");

      // Check that contract received funds
      let newContractBalance;
      newContractBalance = await web3.eth.getBalance(congo.address);
      ...
      const exepectedBalance = oldContractBalance.add(price);

      assert.equal(newContractBalance.toString(), exepectedBalance.toString());
    });
    ...
    it("prevents buyer from tampering with listing & blocks internal func calls", async () => {
      ...
      buyerUpdatesListing = await congo.updateListing(
        id,
        qty,
        price,
        details,
